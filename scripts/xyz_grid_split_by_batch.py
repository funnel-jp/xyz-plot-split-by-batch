...
def confirm_range(min_val: float, max_val: float, axis_label: str):
    """Generates an AxisOption.confirm() function that checks all values are within the specified range."""
...
def draw_xyz_grid(p, xs, ys, zs, x_labels, y_labels, z_labels, cell, draw_legend_x, draw_legend_y, include_lone_images, first_axes_processed, second_axes_processed, margin_size, has_size_axis=False):
    list_size = len(xs) * len(ys) * len(zs)

    processed_result = None
    cell_results = [None] * list_size
    ...
    def process_cell(x, y, z, ix, iy, iz):
        ...
        idx = index(ix, iy, iz)
        state.job = f"{idx + 1} out of {list_size}"
        ...
        # Extract required data and sever reference to original object
        cell_data = {
            "images": [img.copy() for img in processed.images] if processed.images else [],
            "all_prompts": list(processed.all_prompts) if hasattr(processed, "all_prompts") else [],
            "all_seeds": list(processed.all_seeds) if hasattr(processed, "all_seeds") else [],
            "infotexts": list(processed.infotexts) if hasattr(processed, "infotexts") else [],
        }
        cell_results[idx] = cell_data
    ...
    def get_actual_data(pr_data):
        if not pr_data or not pr_data["images"]:
            return [], [], [], []

        imgs = pr_data["images"]
        prompts = pr_data["all_prompts"]
        seeds = pr_data["all_seeds"]
        infos = pr_data["infotexts"]

        if len(imgs) > image_cell_count:
            return (
                imgs[1 : image_cell_count + 1],
                prompts[1 : image_cell_count + 1] if len(prompts) > image_cell_count else prompts,
                seeds[1 : image_cell_count + 1] if len(seeds) > image_cell_count else seeds,
                infos[1 : image_cell_count + 1] if len(infos) > image_cell_count else infos,
            )
        else:
            return imgs[:image_cell_count], prompts, seeds, infos

    cell_mode = "RGB"
    cell_size = (p.width, p.height)
    for pr in cell_results:
        imgs, _, _, _ = get_actual_data(pr)
        if imgs and imgs[0]:
            cell_mode = imgs[0].mode
            cell_size = imgs[0].size
            break

    def create_error_dummy():
        return Image.new(cell_mode, cell_size, color="white")
    ...
    # 1. Build individual z-grids per batch index
    for b in range(image_cell_count):
        b_images = []
        b_prompts = []
        b_seeds = []
        b_infotexts = []
        ...
        for idx in range(list_size):
            imgs, pmpts, sds, infos = get_actual_data(cell_results[idx])

            is_valid_image = (
                b < len(imgs) and imgs[b] is not None and (has_size_axis or imgs[b].size == cell_size)
            )
            if is_valid_image:
                b_images.append(imgs[b])
                b_prompts.append(pmpts[b] if b < len(pmpts) else (pmpts[0] if pmpts else ""))
                b_seeds.append(sds[b] if b < len(sds) else (sds[0] if sds else -1))
                b_infotexts.append(infos[b] if b < len(infos) else (infos[0] if infos else ""))
            else:
                b_images.append(create_error_dummy())
                b_prompts.append("")
                b_seeds.append(-1)
                b_infotexts.append("")
        ...
        # Create sub-grids (z-axis)
        for i in range(z_count):
            start_index = i * len(xs) * len(ys)
            end_index = start_index + len(xs) * len(ys)
            grid = images.image_grid(b_images[start_index:end_index], rows=len(ys))
            if draw_legend_x or draw_legend_y:
                grid_max_w, grid_max_h = map(max, zip(*(img.size for img in b_images[start_index:end_index])))
                b_hor_texts = [[images.GridAnnotation(x)] for x in x_labels] if draw_legend_x else [[images.GridAnnotation()] for _ in x_labels]
                b_ver_texts = [[images.GridAnnotation(y)] for y in y_labels] if draw_legend_y else [[images.GridAnnotation()] for _ in y_labels]
                grid = images.draw_grid_annotations(grid, grid_max_w, grid_max_h, b_hor_texts, b_ver_texts, margin_size)
            b_sub_grids.append(grid)
            b_sub_prompts.append(b_prompts[start_index])
            b_sub_seeds.append(b_seeds[start_index])
            b_sub_infotexts.append(b_infotexts[start_index])

        for i in range(z_count):
            final_images.append(b_sub_grids[i])
            final_prompts.append(b_sub_prompts[i])
            final_seeds.append(b_sub_seeds[i])
            final_infotexts.append(b_sub_infotexts[i])

    # 2. Append all individual cell images for all batches
    for idx in range(list_size):
        imgs, pmpts, sds, infos = get_actual_data(cell_results[idx])
        for b in range(image_cell_count):
            if b < len(imgs) and imgs[b] and (has_size_axis or imgs[b].size == cell_size):
                final_images.append(imgs[b])
                final_prompts.append(pmpts[b] if b < len(pmpts) else (pmpts[0] if pmpts else ""))
                final_seeds.append(sds[b] if b < len(sds) else (sds[0] if sds else -1))
                final_infotexts.append(infos[b] if b < len(infos) else (infos[0] if infos else ""))
            else:
                final_images.append(create_error_dummy())
                final_prompts.append("")
                final_seeds.append(-1)
                final_infotexts.append("")

    processed_result.images = final_images
    processed_result.all_prompts = final_prompts
    processed_result.all_seeds = final_seeds
    processed_result.infotexts = final_infotexts
    processed_result.index_of_first_image = z_count * image_cell_count
...
class Script(scripts.Script):
    def title(self):
        return "X/Y/Z plot split by batch"
    ...
    def ui(self, is_img2img):
        ...
        with gr.Row(variant="compact", elem_id="axis_options"):
            with gr.Column(variant="compact", scale=3):
                with gr.Row(variant="compact"):
                    draw_legend_x = gr.Checkbox(label="Draw legend for X", value=True, elem_id=self.elem_id("draw_legend_x"))
                    draw_legend_y = gr.Checkbox(label="Draw legend for Y", value=True, elem_id=self.elem_id("draw_legend_y"))
                newline_mode = gr.Checkbox(label="Use newline separator", value=False, elem_id=self.elem_id("newline_mode"))
                with gr.Row(variant="compact"):
        ...
        def select_axis(axis_type, axis_values, axis_values_dropdown, csv_mode):
            ...
            return (
                ...
                gr.update(visible=not has_choices or csv_mode, value=axis_values),
        ...
        csv_mode.change(
            ...
            inputs=[csv_mode, x_type, x_values, x_values_dropdown, y_type, y_values, y_values_dropdown, z_type, z_values, z_values_dropdown],
    ...
    def run(
        ...
        def process_axis(opt, vals, vals_dropdown):
            ...
            if opt.type == int:
                ...
                for val in valslist:
                    if str(val).strip() == "":
                        continue
                    m = re_range.fullmatch(str(val))
                    mc = re_range_count.fullmatch(str(val))
                    if m is not None:
                        start = int(m.group(1))
                        end = int(m.group(2)) + 1
                ...
                for val in valslist:
                    if str(val).strip() == "":
                        continue
                    m = re_range_float.fullmatch(str(val))
                    mc = re_range_count_float.fullmatch(str(val))
                    if m is not None:
        ...
        grid_mp = round(len(xs) * len(ys) * len(zs) * p.width * p.height / 3000000)
        ...
        with SharedSettingsStackHelper():
            has_size_axis = any(opt.label == "Size" for opt in [x_opt, y_opt, z_opt])
        ...
        z_count = len(zs)
        grid_count_per_batch = z_count
        image_cell_count = p.n_iter * p.batch_size

        processed.infotexts[:grid_count_per_batch] = grid_infotext[:grid_count_per_batch]
            ...
            if y_type + z_type > 0:
                ...
                if n > 1:
                    ...
                    if draw_legend_x:
                        ...
                        legend_h = font_size + int(font_size * 0.5) + margin_size
                    else:
                        legend_h = 0
                    ...
                    for r in range(1, n + 1):
                        ...
                        total_w = c * w + max(0, c - 1) * margin_size
                        total_h = r * h + max(0, r - 1) * margin_size + legend_h

                        grid_ar = total_w / total_h
                        diff = abs(grid_ar - target_ar)
                    ...
                    target_count = best_r
                    if target_count > 1:
                        logger.debug(f"Automatically split into {target_count} rows for ~16:9 AR")
        else:
            if y_type + z_type == 0:
        ...
        if target_count > 1:
            for b in range(image_cell_count):
                grid_idx = b * grid_count_per_batch
                processed.images[grid_idx] = rearrange_image(processed.images[grid_idx], target_count)

        if opts.grid_save:
            for b in range(image_cell_count):
                for g in range(grid_count_per_batch):
                    idx = b * grid_count_per_batch + g
                    images.save_image(
        ...
        total_grid_images = grid_count_per_batch * image_cell_count

        if not include_lone_images:
            processed.images = processed.images[:total_grid_images]
            processed.all_prompts = processed.all_prompts[:total_grid_images]
            processed.all_seeds = processed.all_seeds[:total_grid_images]
            processed.infotexts = processed.infotexts[:total_grid_images]
